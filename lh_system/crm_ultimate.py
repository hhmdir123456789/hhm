...
def authenticate_user(username, password):
    """验证用户，返回(user_id, username, role)或None"""
    conn = get_db_connection()
    try:
        c = conn.cursor()
...
def require_role(required_role):
    """权限检查装饰器，用于页面函数，但需要session_state中有role"""
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            if st.session_state.get('role') != required_role and required_role == 'admin':
                st.error("您没有权限执行此操作")
                return
        ...
        with st.form("login_form"):
            ...
            if submitted:
                ...
                if user:
                    ...
                    st.error("用户名或密码错误")
        # 简单注册入口（仅管理员可注册？这里只允许普通用户注册，但演示中注册即普通用户）
...
def crm_page():
    ...
    with st.sidebar:
        ...
        if st.session_state.role == 'admin':
            menu_options = [
                "📊 仪表盘", "👥 客户管理", "🔧 工单管理", "📈 销售分析",
                "🗺️ 客户旅程", "🏗️ 工程项目", "💰 项目报价", "📁 数据管理", "🤖 AI助手", "📜 操作日志"
            ]
        else:
            menu_options = [
    ...
    # 项目报价管理
        ...
        if 'viewing_quotation' in st.session_state:
            ...
            # 添加明细（无论角色，都可添加，但可以限制只有admin可操作）
